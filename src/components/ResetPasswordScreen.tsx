...
export function ResetPasswordScreen({ oobCode: _oobCode }: Props) {
  ...
  useEffect(() => {
    ...
    supabase.auth.getSession()
      ...
      .catch(() => {
        if (!mounted) return;
        setError('No se pudo verificar el enlace. Revisa tu conexión e intenta de nuevo.');
        setVerifying(false);
      });
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    if (missing.length > 0) { setError(`La contraseña necesita: ${missing.join(', ').toLowerCase()}.`); return; }
    ...
    <div className="min-h-screen bg-ink-950 flex items-center justify-center p-4">
      <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} className="w-full max-w-md">
        <div className="mb-8 text-center">
          <div className="mx-auto grid h-14 w-14 place-items-center rounded-2xl bg-ink-900/80 ring-1 ring-accent-500/30 shadow-glow">
            <KeyRound size={26} className="text-metal-100" />
          </div>
          <h1 className="mt-4 font-display text-2xl font-medium text-metal-100 tracking-tight">XiX Tech</h1>
        ...
        <div className="card relative p-6">
          ...
          {verifying ? (
            ...
            <div className="flex flex-col items-center gap-3 py-6 text-center">
              ...
              <p className="font-display text-lg font-medium text-metal-100">Contraseña actualizada</p>
              <p className="text-sm text-slate-400">Redirigiendo al inicio de sesión...</p>
            </div>
          ) : (
            <>
              <div className="mb-5">
                <h2 className="font-display text-lg font-medium text-metal-100">Nueva contraseña</h2>
              </div>
              <form onSubmit={submit} className="space-y-4">
                <div>
                  <label className="label"><Lock size={11} className="inline mr-1" />Nueva contraseña</label>
                  <input type="password" required minLength={8} className="input" disabled={loading}
                    value={newPassword} onChange={(e) => setNewPassword(e.target.value)} placeholder="Mínimo 8 caracteres" autoFocus />
