...
const ADMIN_EMAIL = 'xix6urn@gmail.com';
...
export function Header({ onOpenTour, onOpenSidebar, onNavigate, notificationSlot }: { onOpenTour: () => void; onOpenSidebar?: () => void; onNavigate?: (p: import('../types').Permission) => void; notificationSlot?: React.ReactNode; }) {
  ...
  const canSwitchRole = user?.email?.toLowerCase() === ADMIN_EMAIL;
      ...
      <div className="flex h-16 items-center justify-between gap-3 px-4 sm:px-6">
        ...
        <div className="flex items-center gap-2 sm:gap-3">
          ...
          <div className="relative" ref={ref} data-tour="role-switcher">
            ...
            >
              <div
                className={`grid h-7 w-7 place-items-center rounded-lg bg-gradient-to-br ${current.color} text-metal-100 text-xs font-semibold shadow-sm`}
              >
                {current.initials}
              </div>
                ...
                >
                  ...
                  {ROLES.map((r) => {
                    ...
                    return (
                      ...
                      >
                        <div
                          className={`mt-0.5 grid h-8 w-8 shrink-0 place-items-center rounded-lg bg-gradient-to-br ${r.color} text-metal-100 text-[11px] font-semibold shadow-sm`}
                        >
                          {r.initials}
                        </div>
