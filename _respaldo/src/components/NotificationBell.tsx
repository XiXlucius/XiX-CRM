...
export function NotificationBell({ onNavigate }: { onNavigate: (p: Permission) => void }) {
  ...
  return (
    <div className="relative" ref={ref}>
      <button
        onClick={() => setOpen((v) => !v)}
        className="relative grid h-9 w-9 place-items-center rounded-xl border border-tint/10 bg-ink-850/60 text-slate-300 hover:border-accent-500/40 hover:text-metal-100 transition-colors"
      >
        <Bell size={16} />
        {unread > 0 && (
          <span className="absolute -top-1 -right-1 grid h-4 min-w-4 place-items-center rounded-full bg-danger-500 px-1 text-[10px] font-bold text-metal-100">
            {unread > 9 ? '9+' : unread}
          </span>
        )}
      </button>
      ...
      <AnimatePresence>
        {open && (
          ...
          >
            <div className="flex items-center justify-between px-4 py-3 border-b border-tint/5">
              <div>
                <p className="text-sm font-semibold text-metal-100">Notificaciones</p>
            ...
            <div className="flex-1 overflow-y-auto divide-y divide-tint/5">
              {notifications.length === 0 ? (
                ...
                notifications.map((n) => <NotificationItem key={n.id} n={n} onRead={handleRead} onNavigate={onNavigate} onClose={() => setOpen(false)} />)
              )}
            </div>
          </motion.div>
        )}
...
function NotificationItem({ n, onRead, onNavigate, onClose }: { n: AppNotification; onRead: (id: string) => void; onNavigate: (p: Permission) => void; onClose: () => void }) {
  ...
  return (
    <button
      ...
      className={`w-full text-left border-l-2 px-4 py-3 transition-colors hover:bg-tint/[0.03] ${PRIORITY_STYLES[n.priority]} ${!n.read ? '' : 'opacity-60'}`}
    >
      <div className="flex items-start gap-2.5">
        ...
        <div className="flex-1 min-w-0">
          <p className="text-sm font-medium text-metal-100">{n.title}</p>
