...
export function StoreProvider({ children }: { children: ReactNode }) {
  ...
  const seedDemoData = useCallback(async (uid: string) => {
    ...
    const { error: settingsErr } = await supabase.from('business_settings').insert({ user_id: uid, ...DEFAULT_SETTINGS });
  ...
  const refreshAlerts: StoreValue['refreshAlerts'] = async () => {
    ...
    if (newAlerts.length > 0) {
      const { error: insErr } = await supabase.from('notifications').insert(newAlerts.map((a) => ({ ...a, user_id: u.id })));
      if (insErr) throw insErr;
      const { data, error: selErr } = await supabase.from('notifications').select('*').eq('user_id', u.id).order('created_at', { ascending: false }).limit(50);
  ...
  const updateSettings: StoreValue['updateSettings'] = async (patch) => {
    const { data: { user: u } } = await supabase.auth.getUser();
    if (!u) return;
    const { data: existing, error: selErr } = await supabase.from('business_settings').select('id').eq('user_id', u.id).maybeSingle();
    if (selErr) throw selErr;
    if (existing) {
      const { error } = await supabase.from('business_settings').update({ ...patch, updated_at: new Date().toISOString() }).eq('user_id', u.id);
      if (error) throw error;
    } else {
      const { error } = await supabase.from('business_settings').insert({ user_id: u.id, ...DEFAULT_SETTINGS, ...patch });
      if (error) throw error;
    }
